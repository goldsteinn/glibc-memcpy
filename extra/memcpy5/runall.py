...
outfile = "res.txt"
run_cmd = "sudo ./driver -v --core 0 --rt 100000 --func memcpy_dev_v32_movsb --hr --max 32 > " + outfile
...
class Config:
    ...
    def get_content(self):
        content = [
            ...
            "#define ALIGN_ENTRY [ALIGN_ENTRY]", "#define TODO [TODO]",
            "#define EVENT_TO_CHECK [EVENT_TO_CHECK]", "#endif"
        ]
        out = ""
        for c in content:
            out += c.replace("[ALIGN_ENTRY]", self.ae).replace(
                "[TODO]", self.td).replace("[EVENT_TO_CHECK]", self.ev) + "\n"
...
class Run:
    ...
    def run(self):
        ...
        ret = os.system("(cd {}/build; make clean; make)".format(project_path))
        assert ret == 0
        print("Running: {}".format(self.conf.get_key()))
