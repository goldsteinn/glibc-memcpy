...
class Conf():
    def __init__(self, align_movsb, loop_align, movsb_align, order, branch):
    ...
    def write_conf(self):
        ...
        content = [
            ...
            "#define ORDER " + str(self.order), "#endif"
        ]
        f = open(to_path, "w+")
        for c in content:
            f.write(c + "\n")
...
confs = []
for i in range(0, 4):
    confs.append(Conf(1, 64, 64, i, "benchmark-dev"))
    confs.append(Conf(1, 64, 32, i, "benchmark-dev"))
    confs.append(Conf(1, 32, 32, i, "benchmark-dev"))
for c in confs:
    c.write_conf()
