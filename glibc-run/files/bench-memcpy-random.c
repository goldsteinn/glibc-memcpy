...
static void
do_test (json_ctx_t *json_ctx, size_t max_size, int direction)
{
  int i;
  ...
  for (i = 0; i < MAX_COPIES; i++)
    {
      dst_offset  = direction == BIDIRECTIONAL
  ...
  json_attr_string(json_ctx, "direction",
  ...
  for (int i = 4; i <= 512; i = i * 2)
    {
      if (i * 1024 > MAX_TEST_SIZE)
          continue;
      do_test (&json_ctx, i * 1024, FORWARD);
      do_test (&json_ctx, i * 1024, BACKWARD);
      do_test (&json_ctx, i * 1024, BIDIRECTIONAL);
    }
